...
template <typename TriangleQuadratureType, typename ScalarKernelType>
SrcResult SrcSingularity<TriangleQuadratureType, ScalarKernelType>::integrate_TEMP(
    const Complex k,
    const Triangle<2>& src_tri,
    ConstEigRef<EigMatNX<Float, 3>> r_obs
    )
{
    ...
    src_quad_.set_compute_terms(base::compute_g_terms_, base::compute_grad_g_terms_);
    SrcResult result = src_quad_.integrate(k, src_tri, r_obs);
    ...
    for (Index ii = 0; ii < r_obs.cols(); ++ii)
    {
        ...
        // projection of the observation point on the source triangle's local plane, assuming the
        // observation points are in the source's local coordinate system
        EigColVecN<Float, 2> rho_obs = r_obs.col(ii).topRows(2);
        Float z = r_obs(2, ii);
        Float z_abs = std::abs(z);
        Float z_sq = z * z;
        Float z_sign = (0 < z) - (z < 0);
        if (z_abs < TRIANGLE_DEFAULT_TOL * src_tri.shortest_edge_length())
            z_sign = one;

        Float beta = 0;
        Float t0_f2 = 0;
        EigColVecN<Float, 2> u_f2 = EigColVecN<Float, 2>::Zero(2, 1);
        EigColVecN<Float, 2> u_f3 = EigColVecN<Float, 2>::Zero(2, 1);
        ...
        for (uint8_t idx = 0; idx < 3; idx++)
        {
            ...
            // projection of the projected observation point to the line defined by the edge
            EigColVecN<Float, 2> plus_to_rho = rho_obs - edge.v(0);
            EigColVecN<Float, 2> minus_to_rho = rho_obs - edge.v(1);
            Float s_plus = edge.unit_vec().dot(plus_to_rho);
            Float s_minus = s_plus - edge.length();
            Float t0 = -u_hat.dot(plus_to_rho);
            Float R0 = std::sqrt((t0 * t0) + z_sq);

            // distance from the observation points to each end point of the edge
            Float R_plus = std::sqrt(plus_to_rho.squaredNorm() + z_sq);
            Float R_minus = std::sqrt(minus_to_rho.squaredNorm() + z_sq);

            // compute the terms which compose the final integrals
            Float R0_sq = R0 * R0;
            Float atan_plus = std::atan2(t0 * s_plus, R0_sq + z_abs * R_plus);
            Float atan_minus = std::atan2(t0 * s_minus, R0_sq + z_abs * R_minus);

            Float f2 = std::log((R_plus + s_plus) / (R_minus + s_minus));

            if (!std::isfinite(f2))
                f2 = 0;

            Float f3 = s_plus * R_plus - s_minus * R_minus + R0_sq * f2;

            beta += atan_plus - atan_minus;
            t0_f2 += t0 * f2;
            u_f2.array() += u_hat.array() * f2;
            u_f3.array() += u_hat.array() * f3;

            assert(std::isfinite(atan_plus) &&
                   "SrcSingularity::compute_integral_terms(): atan_plus has nan or inf.");
            assert(std::isfinite(atan_minus) &&
                   "SrcSingularity::compute_integral_terms(): atan_minus has nan or inf.");
        ...
        if (base::compute_g_terms_)
        {
            Complex g = (t0_f2 - z_abs * beta) / four_pi;
            result.g[ii] += g;

            result.rs_g.col(ii).array() += 0.5 * u_f3.array() / four_pi;
            result.rs_g.col(ii).array() += r_obs.col(ii).topRows(2).array() * g;
        }

        if (base::compute_grad_g_terms_)
        {
            result.grad_g.col(ii).topRows(2).array() += -u_f2.array() / four_pi;
            result.grad_g(2, ii) += -z_sign * beta / four_pi;

            EigColVecN<Float, 3> grad_r;
            grad_r.topRows(2) = 0.5 * u_f3;
            grad_r[2] = z * z_abs * beta - z * t0_f2;

            result.grad_g.col(ii).array() += (half * k * k) * grad_r.array() / four_pi;
        }

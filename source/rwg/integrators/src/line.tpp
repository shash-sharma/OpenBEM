    ...
    )
{
    ...
    for (uint8_t idx_edge = 0; idx_edge < 3; ++idx_edge)
    {
        ...
        theta_minus.noalias() = GeometryOps<2>::directed_angle_between_vectors(
            h_vec, rproj_to_vminus
            );
        theta_plus.noalias() = GeometryOps<2>::directed_angle_between_vectors(
        ...
        // assert(
        //     (Eigen::abs(
        //         Eigen::abs((theta_plus - theta_minus).colwise().norm().array()) - (angle_between_vectors<2>(rproj_to_vminus, rproj_to_vplus)).array()
        //         ) <= TRIANGLE_DEFAULT_TOL * pi).all() &&
        //         "SrcLineIntegrator::assemble_integrals(): theta_plus and theta_minus are incorrect"
        //     );
